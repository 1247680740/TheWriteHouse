namespace utils {
    ...
    export class Hash {
        ...
        static getRandomNum(min, max): number {
            let num: number = Math.floor(min + Math.floor(Math.random() * (max - min + 1)));
            return num;
        }
		 ...
		 * @param	out 输出坐标。
		 */
        static convert3DCoordTo2DScreenCoord(source: Laya.Vector3, out: Laya.Vector3): Laya.Vector3 {
        ...
        static conver3DTo2DCoord(source: Laya.Vector3, out: Laya.Vector3, sp: Laya.Sprite3D): Laya.Vector3 {
            // let newVec:Laya.Vector3 = new Laya.Vector3();
            // newVec = out;
            let viewPort: Laya.Viewport = new Laya.Viewport(0, 0, 520, 500);
		 ...
		 * @param	out 输出坐标。
		 */
        static screenCoordTo3DCoord(source: Laya.Vector3, out: Laya.Vector3): Laya.Vector3 {
            var se: Float32Array = source.elements;
            var oe: Float32Array = out.elements;
        ...
        /** 判断某点是否包含在多边形内 */
        static pointInPoly(pt, poly): boolean {
        ...
        /** 百分比几率计算 */
        static countRate(probability: number): number {
            probability = probability * 100;
            let odds: number = Math.floor(Math.random() * 100);
        ...
        /** 判断两个日期之间相差天数 */
        static dateDifference(data1, data2): number {
        ...
        /** 播放背景音乐/音效 */
        static playMusic(soundId: number): void {
            switch (soundId) {
                case 1:
